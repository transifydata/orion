...
export function getClosestStopTimes(
  ...
  return getScheduledVehicleLocationsSQL(
    time,
    db,
    timeOfDay,
    timeOfDayBefore,
...
export function processClosestStopTimes(
  ...
  function interpolateLocationSimpleLinear(
    beforeStopTime: ClosestStopTime,
    afterStopTime: ClosestStopTime,
  ) {
    ...
    const timeDiff =
      ...
      HHMMSSToSeconds(beforeStopTime.departure_time);
    const currentTimeDiff =
      timeOfDaySecs - HHMMSSToSeconds(beforeStopTime.departure_time);
  ...
  function interpolateLocationAlongShape(
    beforeStopTime: ClosestStopTime,
    afterStopTime: ClosestStopTime,
  ) {
    ...
    const currentShapeDistTravelled =
      beforeShapeDistTravelled +
      interpolationFactor *
        (afterShapeDistTravelled - beforeShapeDistTravelled);
    ...
    if (currentShapeDistTravelled + 0.005 < 1.0) {
      ...
      const deltaPosition = shape.interpolate(
        currentShapeDistTravelled + 0.005,
      );
  ...
  const result = stopTimePairs.map(([beforeStopTime, afterStopTime]) => {
    ...
    if (
      isDefined(beforeStopTime.shape_dist_traveled) &&
      isDefined(afterStopTime.shape_dist_traveled)
    ) {
      shapeInterpCount += 1;
  ...
  console.log(
    `Used shape interpolation for ${shapeInterpCount} / ${stopTimePairs.length} buses`,
  );
  return result;
}
...
function convertClosestStopTimeToVehiclePositions(
  ...
  const { route_id: routeid, trip_headsign } = db.getTrips(
    { trip_id: st.trip_id },
...
export async function getScheduledVehicleLocations(
  ...
  const closestStopTimes = getClosestStopTimes(
    gtfsDatabase,
    new Date(time),
    undefined,
  );

...
export default async function getVehicleLocations(agency: string, time: number): Promise<LinkedPositionsOutput> {
    const {result: scheduledPositions, time: scheduledTime} = await measureExecutionTime(async () => await getScheduledVehicleLocations(agency, time));
    const {result: livePositions, time: liveTime} = await measureExecutionTime(async () => await getLiveVehicleLocations(agency, time));

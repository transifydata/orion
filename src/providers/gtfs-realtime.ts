...
export async function getTripUpdates(
  config: Agency,
): Promise<GtfsRealtimeBindings.transit_realtime.TripUpdate[]> {
  ...
  return feed.entity.map((item) => {
    if (!item.tripUpdate) {
      throw new Error(
        "Unexpected FeedEntity in TripUpdates - " + JSON.stringify(item),
      );
    }
    return new GtfsRealtimeBindings.transit_realtime.TripUpdate(
      item.tripUpdate,
    );
  });
}
      ...
      if (error) {
        ...
        reject(
          new Error(
            "HTTP " +
              response.statusCode +
              " fetching gtfs-realtime feed from " +
              url,
          ),
        );
      }
